...
const Dashboard: React.FC = () => {
  const { showSidePanel, setShowCreateItemModal, items } = useAppContext();

  return (
    <div>
      <div className="flex justify-between md:mb-10 my-6 md:my-0 px-4 md:px-0">
        <div className="">
      ...
      <div className="flex flex-row md:space-x-8">
        <div
          className={`rounded-2xl py-5 px-4 sm:px-6 bg-[#1e1f25] h-fit transition-all ease-linear ${
            showSidePanel ? "w-0 md:w-3/5 hidden md:block" : "w-full"
          }`}
        >
        ...
        <div
          className={`rounded-2xl py-5 px-4 sm:px-6 bg-[#1e1f25] h-fit transition-all ease-in-out duration-100 ${
            showSidePanel ? "w-full md:w-2/5" : "w-0 hidden"
          }`}
        >
          <SidePanel />
        </div>
